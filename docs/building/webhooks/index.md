...
Both flavors plug into the same fan-out: `Event.emit()` walks every matching subscriber and enqueues one delivery task each. Delivery rides the [task adapter](../tasks/index.md) — the retry schedule, durability, and worker model are all the task system's, not bespoke webhook code.
