...
Causeway accepts both Next.js-style folders and TanStack-style dotted leaves in the same tree.
...
3. **The bracketed segment binds by name.** `[id].py` requires the handler to take `id` (not `user_id`, not `pk`).

## Why brackets work as filenames

Python doesn't allow `[`, `]`, or `$` in module names, so Causeway loads route files via `importlib.util.spec_from_file_location()` rather than ordinary `import` statements. The filename is just a string on disk; the import machinery never sees the brackets as attribute lookups.
...
- [Dynamic segments](./dynamic-segments.md) — `[id]`, `$id`, type binding.
