...
This is the long version of the story in the README — why Quay exists, what it deliberately won't do, and how it compares to everything in the same lane. If you only want to know "what is this thing" you can skim the headings.
...
So I built **Quay** — the layer that turns dyadpy into something you can build a product on.

**dyadpy** is the typed-RPC engine. **Quay** is the framework around it: file-based routing, scoped DI, middleware composition, a plugin registry, a background-task contract, observability, and the CLI that ties it all together. You write Python handlers; Quay registers them with dyadpy, dyadpy emits the TypeScript client, and your frontend gets exactly the types your server produces.
...
Quay is what those preferences look like when you actually sit down and build them.
...
1. **No ORM in core.** Ever. The `quay-sqlmodel` plugin (and any future `quay-tortoise` / `quay-prisma-py`) lives outside.
2. **No admin panel in core.** Recommend `sqladmin` / `Dashibase` / `retool` / `RowZero`. Maybe a `quay-admin-*` plugin family someday.
...
4. **No infrastructure provisioning.** That's Terraform / Pulumi / Modal. `quay deploy` shells out to provider CLIs; it does not manage your infrastructure.
