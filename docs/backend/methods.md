...
Method decorators turn a function into a handler. They do not take a path — the file router already knows the path from the file location.
...
The only route contract option on method decorators is `refreshes`.
...
`refreshes` lists route keys that should be refreshed after a successful mutation. It is metadata: request execution does not change, and failed mutations do not trigger refreshes. The generated client runtime reads the contract and refreshes matching cached queries.

Use it when the mutation naturally knows what it made stale. Leave it off when the result is self-contained or the frontend owns a more specific update.
...
- [Reference — decorators](../reference/decorators/get.md)
