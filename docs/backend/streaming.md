...
- **Background tasks + result polling** — for long jobs the client kicks off and waits for. The `@task` decorator handles enqueue; a separate `GET /jobs/{id}` polls. See [Tasks](../app/tasks.md).
...
- [Tasks](../app/tasks.md) — for background work, not request-shaped streams.
- [Reference — `stream`](../reference/functions/stream.md)
