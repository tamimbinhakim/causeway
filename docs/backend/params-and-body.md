...
See [Dynamic segments](./dynamic-segments.md) for the full set of supported types.
...
For headers you read on every request (auth tokens, idempotency keys), promote them to a [scoped provider](./scopes.md) so the handler stays clean.
...
The file router rewrites `Annotated[Session, get_session]` into `causeway.Depends(get_session)` at boot, so handler code stays declarative. See [Scopes](./scopes.md).
