...
6. Mounts `/healthz`, `/readyz`, `/__causeway` (diagnostics page), and `/__causeway/graph` (dev-only App Graph JSON).

## Diagnostics page

Open <http://127.0.0.1:8000/__causeway>:

- Route tree (route keys, handler source paths, declared errors, providers in scope).
- App Graph JSON at `/__causeway/graph`.
