...
| `error_formatter`  | `None`              | Optional app-level formatter for `HttpError` payloads in both typed envelopes and problem+json.               |
...
5. Builds and attaches the App Graph to the inner runtime app.
6. Attaches `/healthz` and `/readyz` unconditionally; `/__causeway` and dev-only `/__causeway/graph` when `diagnostics=True`.
7. Wraps the inner runtime `App` in a Starlette app with:
...
8. Walks the three event-related trees:
...
9. Wires lifespan: app `lifespan.py` startup, plugin startup, then every `_scope.py` `startup()`; shutdown runs the route hooks in reverse, then plugins, then app `lifespan.py`.
...
- [Events overview](../../app/events.md)
- [Webhooks overview](../../app/webhooks.md)
- [Subscribers overview](../../app/subscribers.md)
