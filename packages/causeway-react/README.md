...
> React hooks for [Causeway](https://github.com/tamimbinhakim/causeway)
> generated clients, built on
> [TanStack Query](https://tanstack.com/query/latest).

```bash
pnpm add @causewayjs/react @causewayjs/ts @tanstack/react-query
```

`@causewayjs/react` adds hook leaves on top of your generated client:
...
import { createReactClient } from "@causewayjs/react";
...
Use `@causewayjs/react/server` from React Server Components or other server
loaders to prefetch into a TanStack `QueryClient`. Create a request-scoped
Causeway client so auth headers and the server `baseUrl` do not leak across
requests:

```tsx
import { dehydrate, QueryClient } from "@tanstack/react-query";
import { prefetchQuery } from "@causewayjs/react/server";
import { forwardHeaders } from "@causewayjs/ts";
...
Causeway routes decorated with `@raises(...)` return a `Result<T, E>`
envelope. `@causewayjs/react` unwraps it automatically:
...
- `@causewayjs/ts` (workspace; required by your generated client)
