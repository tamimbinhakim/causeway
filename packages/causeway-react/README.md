...
```bash
pnpm add causeway-react causeway-ts @tanstack/react-query
```

`causeway-react` adds hook leaves on top of your generated client:
...
Use `causeway-react/server` from React Server Components or other server
...
import { prefetchQuery } from "causeway-react/server";
import { forwardHeaders } from "causeway-ts";
...
envelope. `causeway-react` unwraps it automatically:
...
- `causeway-ts` (workspace; required by your generated client)
