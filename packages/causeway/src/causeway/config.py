...
Causeway's ``Settings`` is just ``pydantic_settings.BaseSettings`` with one wrapper
...
The ``causeway.toml`` file is read separately by :func:`load_manifest`. The only
...
class Manifest:
    """Parsed ``causeway.toml``. Fields default to empty when the file is absent."""

    expose_settings: tuple[str, ...] = ()
    app: dict[str, Any] = field(default_factory=_empty_app)


def load_manifest(path: str | Path = "causeway.toml") -> Manifest:
    """Read ``causeway.toml`` and return a :class:`Manifest`.
