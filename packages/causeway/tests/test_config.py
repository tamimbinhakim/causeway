"""Config + causeway.toml manifest tests."""
...
from causeway.config import Manifest, expose_for_client, load_manifest
...
def test_missing_manifest_returns_default(tmp_path: Path) -> None:
    assert load_manifest(tmp_path / "causeway.toml") == Manifest()


def test_parses_expose_settings(tmp_path: Path) -> None:
    m = tmp_path / "causeway.toml"
    _write(
        m,
