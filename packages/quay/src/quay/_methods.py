...
Handlers in route files use the bare decorator; the file router (``quay.routing``)
...
with ``__quay_method__`` so discovery can find it.
"""
...
def _mark(method: HttpMethod) -> Callable[[Handler], Handler]:
    def decorator(handler: Handler) -> Handler:
        existing = getattr(handler, "__quay_method__", None)
        if existing is not None and existing != method:
            ...
            raise TypeError(msg)
        handler.__quay_method__ = method  # type: ignore[attr-defined]
        return handler
...
def method_of(obj: Any) -> HttpMethod | None:
    ...
    return getattr(obj, "__quay_method__", None)
