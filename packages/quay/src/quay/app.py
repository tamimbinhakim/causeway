...
:func:`create_app` is the single entry point a Quay project uses. It
...
from quay.diagnostics import attach as attach_diagnostics
from quay.errors import error_renderer
from quay.health import attach as attach_health
from quay.middleware import Middleware as QuayMiddleware
from quay.observability import RequestIdMiddleware
...
def create_app(
    ...
    *,
    ...
    diagnostics endpoint (``/__quay``) is opt-out via ``diagnostics=False``.
    """
    inner = App()
...
def _collect_class_middleware(found: Any) -> list[QuayMiddleware]:
    ...
    """
    seen: dict[int, QuayMiddleware] = {}
    for route in found.routes:
        recorded = getattr(route.handler, "__quay_class_middleware__", None)
        if not recorded:
            continue
        for mw in recorded:
