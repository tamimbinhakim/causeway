...
def test_env_defaults_to_dev(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("QUAY_ENV", raising=False)
    monkeypatch.delenv("ENV", raising=False)
    assert env() == "dev"


def test_env_reads_quay_env_first(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ENV", "prod")
    monkeypatch.setenv("QUAY_ENV", "staging")
    assert env() == "staging"
