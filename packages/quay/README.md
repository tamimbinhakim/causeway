...
> A lean backend framework on top of Dyadpy.
...
`quay dev` discovers the routes, registers them into Dyadpy, boots uvicorn, regenerates the TypeScript client on every save, and exposes `/__quay` with the route tree, registered tasks, current config, and plugin list.
...
| `quay` CLI                                       | `new`, `dev`, `build`, `deploy <target>`.                             |
